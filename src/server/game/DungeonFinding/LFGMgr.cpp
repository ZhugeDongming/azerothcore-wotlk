...
#ifdef NPCBOT  //update bots' roles
...
#ifdef NPCBOT  //handle player's bots
        if (player->HaveBot())
        {
            ...
            {
                uint8 pcount = 0;
                for (GroupReference const* gitr = grp->GetFirstMember(); gitr != NULL; gitr = gitr->next())
                    if (gitr->GetSource())
                        ++pcount;
                if (pcount <= 1)
                {
